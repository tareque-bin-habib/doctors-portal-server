...
function verifyJWt(req, res, next) {
    ...
    if (!authHeader) {
        return res.status(401).send('unaouthorized access')
    }
    ...
    try {
        ...
        app.post('/bookings', async (req, res) => {
            ...
            if (alreadyBooked.length) {
                const message = `You Have already an a booking on ${booking.appointmentDate}`
...
app.get('/', async (req, res) => {
    res.send('doctor porta; server is running');
});
